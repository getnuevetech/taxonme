...
import { backfillSituationQaIntelligenceAction } from "@/actions/admin";

export function SituationQaIntelligenceBackfillPanel() {
  const [state, formAction, pending] = useActionState(backfillSituationQaIntelligenceAction, null);

  return (
    <div className="rounded-xl border border-slate-200 bg-white px-4 py-4 shadow-sm">
      <h2 className="text-sm font-semibold text-slate-900">Backfill empty Situation / Q&A intelligence</h2>
      <p className="mt-1 text-xs text-slate-500">
        Package W — writes Package I/R enrichment only for Situations or QaThreads with empty or unparseable{" "}
