...
export function UpdatesSection({
  ...
  seeAllHref = "/irs-updates",
  /** Homepage teaser shows intro + “view all”. Full /irs-updates list is items only. */
  variant = "teaser",
}: {
  ...
  return (
    <section id="irs-updates" className="border-b border-slate-200 bg-[#fbfaf7]">
      <div className={`mx-auto max-w-6xl px-4 ${isList ? "py-10" : "py-20"}`}>
        {!isList && (
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div className="max-w-xl">
              <Kicker>{agencyLabel} updates</Kicker>
        ...
        <div className={`${isList ? "" : "mt-10"} divide-y divide-slate-200 border-t border-slate-200`}>
          {items.map((item) => (
            <article key={item.slug} className="grid gap-2 py-7 md:grid-cols-[140px_1fr] md:gap-8">
              ...
              <div>
                <h3 className="font-serif text-2xl font-bold text-slate-900">
                  <Link href={`/irs-updates/${item.slug}`} className="hover:text-indigo-600">
                    {item.title}
                  </Link>
                </h3>
                {item.summary && (
